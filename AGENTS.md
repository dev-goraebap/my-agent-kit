...
이 저장소는 **마켓플레이스(`dev-goraebap`) = 여러 플러그인 번들**의 구조를 가진다. 각 플러그인은 스킬 묶음이고, 스킬은 이 저장소의 최소 작업 단위다. **플러그인은 저장소 루트 바로 아래**에 둔다(중간 `plugins/` 래퍼 없음).
...
- **`agents-md-kit`** — AGENTS.md 작성·큐레이션·유지보수 도구 모음 (공개 트랙)
- **`misc`** — 어디에도 안 속하는 잡동사니 스킬 모음 (개인 트랙)
...
- 플러그인 이름은 **범용적이고 결합도 낮은** 이름을 쓴다 (예: `agents-md-kit`). 저장소 이름(`my-agent-kit`)과 겹치지 않도록 한다.
...
# /plugin install <plugin-name>@dev-goraebap
```
