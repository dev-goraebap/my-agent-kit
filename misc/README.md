...
**Material Design 3 공식 가이드라인** 지식 스킬. m3.material.io 전체(foundations·styles·components·develop)를 117페이지 로컬 문서로 보관합니다. SKILL.md는 라우팅 테이블 역할만 하고, 실제 내용은 `references/` 하위에 원본 구조 그대로 분할되어 필요할 때만 로드됩니다 (Progressive Disclosure).

**사용 예:**

```
M3 Color Roles가 뭐야?
Window Size Classes 기준으로 레이아웃 나눠줘
머테리얼3에서 Motion easing 토큰 종류는?
M3 Button 컴포넌트 변형 종류 알려줘
```
...
| 섹션 | 페이지 수 | 주요 내용 |
|------|---------|---------|
| `foundations/` | 40p | 레이아웃(16p) ⭐, 디자인 토큰, 인터랙션 States, 적응형 디자인, 용어 사전 |
| `styles/` | 35p | Color System·Roles·Dynamic Color(14p) ⭐, Typography Scale, Shape, Elevation, Motion |
| `components/` | 38p | 38개 컴포넌트 overview (Buttons, Cards, Navigation, Dialogs 등) |
| `develop/` | 4p | Android(Compose·MDC), Flutter, Web 플랫폼 가이드 |

**원본:** https://m3.material.io (수집일: 2026-04-29, Chrome SPA 자동 순회)
